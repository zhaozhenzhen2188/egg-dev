...
class HomeController extends Controller {
    ...
    async getDataForCate() {
        const ctx = this.ctx;
        const params = ctx.params
        if (ctx.query.contentType == 1) {
            // 模板
            if (params.cate2) {
                ctx.tempPage = `${params.cate0}/${params.cate1}/${params.cate2}.html`;
            } else if (params.cate1) {
                ctx.tempPage = `${params.cate0}/${params.cate1}.html`;
            } else if (params.cate0) {
                ctx.tempPage = `${params.cate0}.html`
            } else {
                ctx.tempPage = `404.html`
            }

            ctx.pageType = 'template_cate'
        } else {
            // 内容
            ctx.pageType = 'cate'
        }
