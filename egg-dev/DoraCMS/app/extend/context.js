...
module.exports = {
    ...
    async getPageData() {
        ...
        if (targetTempPage.indexOf('users/') == '0') {
            ...
            if (fs.existsSync(themePath + targetTempPage)) {
                await ctx.render(defaultTemp.alias + '/' + targetTempPage, pageData);
            } else { 
                console.log(themePath + targetTempPage,'页面不存在') 
                await ctx.render(defaultTemp.alias + '/404.html', pageData);
